...
// All fetching is server-side to avoid CORS and keep API keys off the client
...
function formatReleaseNotes(body) {
  if (!body) return '';
  // Strip markdown links, collapse whitespace, truncate
  return body
    .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
    .replace(/#{1,6}\s*/g, '')
    .replace(/\*\*/g, '')
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Recent commit messages (deduplicated noise)
    ...
    // Release velocity: releases in the last 30 days
    ...
    if (anthropicKey && (formattedReleases.length > 0 || changelog.length > 0)) {
      ...
      try {
        ...
        // summary stays null — not critical
      }
    }
